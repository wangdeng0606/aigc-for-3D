import { X, Video, Sparkles, Play, Loader2, Scissors, Upload, User, Check, ImageIcon, FileText, Settings, Zap, ArrowRight, AlertCircle } from 'lucide-react';
import { useState, useMemo } from 'react';

export default function VideoPanel({ node, characters, allNodes, allEdges, relationships, onPushEvents, onUpdateCharacter, onUpdate, onClose }) {
  const d = node.data;
  ...
  /* AI 分镜生成状态 */
  const [storyboardGenStatus, setStoryboardGenStatus] = useState('idle'); // idle | generating
  ...
  const involvedCharIds = useMemo(() => {
    ...
    for (const nid of connectedIds) {
      ...
      if (n?.data?.characterIds) n.data.characterIds.forEach((id) => charIdSet.add(id));
    }
    return [...charIdSet];
  }, [allEdges, allNodes, node.id]);

  const involvedChars = (characters || []).filter((c) => involvedCharIds.includes(c.id));

  /* 收集场景关键词作为上下文 */
  const sceneContext = useMemo(() => {
    if (!allEdges || !allNodes) return '';
    const connectedSceneIds = allEdges
      .filter((e) => e.source === node.id || e.target === node.id)
      .map((e) => (e.source === node.id ? e.target : e.source));
    const parts = [];
    for (const sid of connectedSceneIds) {
      const sNode = allNodes.find((n) => n.id === sid);
      if (sNode?.data?.keywords) parts.push(sNode.data.keywords);
      if (sNode?.data?.title) parts.push(sNode.data.title);
    }
    return parts.join('；');
  }, [allEdges, allNodes, node.id]);
  ...
  /* ===== AI 人物事件分析 ===== */
  const runAnalysis = () => {
    if (!involvedCharIds.length) return;
    const context = [d.script, d.backgroundSetting, d.storyboard, sceneContext].filter(Boolean).join('；');
    if (!context) return;
    ...
    setTimeout(() => {
      const suggested = [];
      const ts = Date.now();
      for (let i = 0; i < involvedCharIds.length; i++) {
        for (let j = i + 1; j < involvedCharIds.length; j++) {
          const a = characters.find((c) => c.id === involvedCharIds[i]);
          const b = characters.find((c) => c.id === involvedCharIds[j]);
          if (!a || !b) continue;
          ...
          suggested.push({
            ...
            sourceCharId: a.id, targetCharId: b.id,
            title: '争夺武林秘笈',
            desc: `双方同时发现藏匿于竹林中的武功秘笈，${a.name}与${b.name}为争夺秘笈展开激烈交锋`,
          ...
          suggested.push({
            ...
            sourceCharId: a.id, targetCharId: b.id,
            title: '得知杀师仇人身份',
            desc: `交手中${a.name}认出${b.name}所用招式正是当年杀害自己师父的凶手所使之术`,
            tag: '冲突', type: 'event', accepted: true,
          });
          suggested.push({
            id: `sug-${ts}-${i}-${j}-rel`,
  ...
  const handleGenerate = async () => {
    ...
    // 收集参与角色的详细信息（name, role, description, tags）
    ...
    // 查找第一个有音频的角色，取其 audio URL
    const firstAudioChar = involvedChars.find((c) => {
      const phase = c.phases?.find((p) => p.id === c.activePhase) || c.phases?.[0];
      return phase?.audio;
    });
    const audioUrl = firstAudioChar
      ? (firstAudioChar.phases?.find((p) => p.id === firstAudioChar.activePhase) || firstAudioChar.phases?.[0])?.audio
      : null;
    ...
    try {
      const res = await fetch('http://localhost:8080/api/video/generate', {
        method: 'POST',
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-5">
          ...
          <div className="w-full h-44 rounded-xl border border-slate-200/60 bg-gradient-to-br from-slate-50 to-violet-50/20 flex items-center justify-center overflow-hidden">
            {d.status === 'generating' ? (
              ...
              <div className="text-center">
                ...
                <div className="text-[10px] text-slate-400 font-medium">填写剧本后生成</div>
              </div>
            )}
          </div>
        </section>
        ...
        <section>
          ...
          <textarea
            ...
            placeholder="当前镜头的剧本内容，如：沈逸风持剑质问楚夜寒杀师之仇，楚夜寒冷笑回应…"
          />
        </section>
        ...
        <section>
          ...
          <textarea
            className="input resize-none"
            rows={2}
            value={d.backgroundSetting || ''}
            onChange={(e) => update('backgroundSetting', e.target.value)}
            placeholder="场景背景与氛围，如：深夜竹林，月光映照，风声萧瑟，紧张对峙气氛…"
          />
        </section>
        ...
        <section>
          ...
          <textarea
            ...
            placeholder="基于剧本与背景设定，AI 自动生成分镜描述，也可手动编辑…"
          />
          {!d.script?.trim() && !d.backgroundSetting?.trim() && (
            <div className="text-[9px] text-slate-400 mt-1.5">请先填写「剧本」或「背景设定」，AI 才能生成分镜</div>
          )}
        </section>
        ...
        >
          {d.status === 'generating' ? (
            ...
            <><Sparkles size={13} /> 生成视频</>
          )}
        </button>

        {/* ===== AI 人物事件分析（从 ScenePanel 迁移） ===== */}
        <section className="border-t border-slate-100/80 pt-5">
          ...
          {analysisStatus === 'idle' && (
            <button
              onClick={runAnalysis}
              disabled={!involvedCharIds.length || (!d.script?.trim() && !d.backgroundSetting?.trim() && !d.storyboard?.trim())}
              className={`w-full btn ${
                !involvedCharIds.length || (!d.script?.trim() && !d.backgroundSetting?.trim() && !d.storyboard?.trim())
                  ? 'bg-slate-100 text-slate-400 cursor-not-allowed'
          ...
          <section className="border-t border-slate-100/80 pt-5">
            ...
            <div className="bg-gradient-to-r from-rose-50/50 to-pink-50/30 border border-rose-100/60 rounded-xl p-3 mb-3">
              <div className="text-[10px] text-rose-600/80 leading-relaxed">
                从视频中截取角色的关键帧图片（正面/侧面/背面），选择对应角色后 AI 将生成新的角色形象参考图，确保后续场景中人物一致性。
              </div>
            </div>
